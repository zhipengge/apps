# 牧云图片 (MuyunImage) — App Store 发布素材
...
```
首发：内容感知消除 + 本地 AI 消除双引擎、滤镜调色、标注形状、透视校正、马赛克。原生 SwiftUI，零追踪，图片永不离开你的 Mac。
```
**字符数：61 / 170 ✅**
...
```
图片编辑,消除,去水印,抠图,修图,标注,截图,马赛克,滤镜,裁剪,透视,批注,箭头,照片
```

### 🇺🇸 English (约 97 / 100 字符)

```
photo,editor,remove,object,eraser,retouch,annotate,mosaic,filter,crop,watermark,inpaint,markup
```

### 🔁 关键词选取依据
- **高搜索量**：`消除` / `去水印` / `remove` / `eraser`（消除类核心词）
- **场景词**：`截图` / `标注` / `马赛克` / `annotate` / `markup`（办公用户搜索习惯）
- **功能词**：`滤镜` / `裁剪` / `filter` / `crop` / `inpaint`
- **避开**：不出现 "Photoshop" / "Pixelmator" / "CleanShot" 等其他 App 名
...
依据：无账号、无分析、无广告、图片不上传；唯一网络请求（模型下载）不携带任何用户数据或标识符。
...
[ ] 1. App 名称        牧云图片 (≤30 字符)
[ ] 2. 副标题          见下方建议
[ ] 3. 推广文本        中文 61 / 英文 150  (≤170)
[ ] 4. 描述            中文 ~850 / 英文 ~1900  (≤4000)
[ ] 5. 关键词          中文 ~96 / 英文 ~97  (≤100)
[ ] 6. 支持 URL        必填，替换占位
[ ] 7. 营销 URL        建议填，替换占位
[ ] 8. 隐私政策 URL    托管 privacy.html 后填入，必填
[ ] 9. App 隐私问卷    选 "不收集数据"
[ ] 10. App 类别       主类别：摄影与录像 (Photography) / 次类别：图形和设计
[ ] 11. 内容评级       4+（无暴力/色情/赌博/医药）
[ ] 12. 出口合规       已在 Info.plist 配置 ITSAppUsesNonExemptEncryption = NO（见 EXPORT_COMPLIANCE.md）
[ ] 13. 截图           macOS 需 1280×800 / 1440×900 / 2560×1600 / 2880×1800 之一，3-10 张
```

### 副标题建议（30 字符内，显示在 App 名下方）

🇨🇳 中文：`本地 AI 消除 · 隐私优先的修图工具` （17 字符）
🇺🇸 English：`On-device AI object removal` （27 字符）
...
1. **消除前后对比** — 一张有路人/杂物的照片，涂抹区域高亮 + 消除后的干净结果
2. **AI 消除进行中** — 展示进度条与笔刷涂抹交互
3. **标注工具** — 截图上加箭头、文字、形状的效果
4. **滤镜与调色** — 右侧面板展开滤镜堆栈与调色滑杆
5. **透视校正** — 四点框选一张文档照片
6. **马赛克** — 给敏感信息打码的场景
...
> ⚠️ **首次上架重要提醒**
>
> 1. AI 功能审核较严：审核备注（App Review Information → Notes）里写明 **AI 消除为本地 CoreML 推理，模型从 GitHub 开源项目下载，无任何云端 API、无生成式内容风险**，可显著降低被问询概率。
> 2. 描述里**不要**出现其他 App 名称（Photoshop 等），会被拒。
> 3. 描述/关键词不要用"无限"、"最强"、"最好"等绝对化表述。
> 4. 沙盒 App 首次审核可能被要求演示文件访问流程，截图中最好体现标准的打开/保存面板。
